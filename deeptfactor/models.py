...
class DeepTFactor(nn.Module):
    ...
    def forward(self, x):
        #print("X size: ", x.size())  #[32, 1, 2000, 21]

        x = self.cnn0(x)
        #print("X size: ", x.size())  #[32, 384, 1001, 1]

        x = x.view(x.size(0), -1)
        #print("X size: ", x.size())  #[32, 384384]
        ...
        x = x.view(-1, self.fc2.out_features)
    ...
    def make_subnetwork(self, subnetwork):
        ...
        for i, kernel in enumerate(subnetwork):
            if i == 0:
                subnetworks.append(
                    nn.Sequential(
                        nn.Conv2d(in_channels=1, out_channels=128, kernel_size=(kernel, 22)),
