...
ax.scatter(a.value[0].detach().cpu().numpy(), a.value[1].detach().cpu().numpy(), color="blue")
ax.set_title("Illustrating the similarities of each point to a")
for i, sim_to_a in enumerate(similarities_to_a):
    plt.plot([points[i,0], a.value[0].detach().cpu().numpy()], [points[i,1], a.value[1].detach().cpu().numpy()], alpha=float(sim_to_a.detach().cpu().numpy()), color="blue")
plt.show()
